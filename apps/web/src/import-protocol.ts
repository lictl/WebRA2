...
export function validProgress(value: unknown): value is ImportProgress {
  return record(value) && (Object.getPrototypeOf(value) === Object.prototype || Object.getPrototypeOf(value) === null) && Reflect.ownKeys(value).every(key => typeof key === 'string' && ['phase', 'filesProcessed', 'totalFiles', 'archives', 'members', 'bytesRead', 'currentPath'].includes(key)) && (value.phase === 'validate' || value.phase === 'archives' || value.phase === 'requirements') &&
