...
export const loadMissionScene:SceneLoader=async(files,profile,progress)=>{
  ...
  try{
    ...
    const voxels=await prepareVoxelPreview(catalog,voxelPlan,{anchors:Object.freeze([...roots.values()]),onProgress(p){progress({phase:p.phase==='verify'?'voxel-sources':'voxels',completed:p.completed,total:p.total,bytes:verifiedBytes});}});
