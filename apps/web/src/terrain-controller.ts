...
export class TerrainController{
  ...
  state:TerrainState;
  constructor(locale:Locale='en',private factory:()=>TerrainPort=()=>new TerrainBridge()){this.state={locale,profile:'ra2',phase:'empty',files:0,bytes:0,busy:false,progress:null,frame:null,cell:null,notice:'choose',error:null};}
  ...
  select(files:ArrayLike<File>):void{
    this.#stop();if(!Number.isSafeInteger(files.length) || files.length<0 || files.length>VIEW_LIMIT.files){this.#files=[];this.#update({phase:'failed',files:0,bytes:0,busy:false,progress:null,frame:null,cell:null,notice:'tooMany',error:null});return;}
    this.#files=Array.from(files);this.#update({phase:files.length?'selected':'empty',files:files.length,bytes:this.#files.reduce((n,f)=>n+f.size,0),busy:false,progress:null,frame:null,cell:null,notice:files.length?'selected':'choose',error:null});
  }
  setLocale(locale:Locale):void{if(locale==='en'||locale==='zh-Hant')this.#update({locale});}
  setProfile(profile:TerrainProfile):void{if((profile!=='ra2'&&profile!=='yr')||profile===this.state.profile)return;this.#stop();this.#update({profile,phase:this.#files.length?'selected':'empty',frame:null,cell:null,progress:null,busy:false,notice:this.#files.length?'selected':'choose',error:null});}
  cancel():void{if(!this.state.busy)return;this.#stop();this.#update({phase:'cancelled',frame:null,cell:null,progress:null,busy:false,notice:'cancelled',error:null});}
  leave():void{this.#stop();this.#update({phase:this.#files.length?'selected':'empty',frame:null,cell:null,progress:null,busy:false,notice:this.#files.length?'selected':'choose',error:null});}
  async load():Promise<void>{
    ...
    this.#update({phase:'loading',frame:null,cell:null,busy:true,progress:null,notice:'loading',error:null});
    try{
  ...
  #failure(e:unknown,generation:number):void{if(generation!==this.#generation)return;this.#stop();this.#update({phase:'failed',busy:false,progress:null,frame:null,cell:null,notice:'failure',error:e instanceof Error?e.message:'unavailable'});}
  ...
  #camera(camera:Camera):void{if(!validCamera(camera))return;this.#desired=camera;this.#update({cell:null});void this.#render();}
  ...
  async pick(x:number,y:number):Promise<void>{
    ...
    try{
      ...
      this.#update({busy:false,...(cameraCurrent?{cell:result.cell,notice:result.cell?'picked':'background'}:{})});if(!cameraCurrent)void this.#render();
    }catch(e){this.#failure(e,generation);}
  }
  dispose():void{this.#stop();this.#files=[];this.state={...this.state,frame:null,cell:null};this.#listeners.clear();}
}
