...
export function attachTerrainWorker(scope: TerrainScope, load: SceneLoader): void {
  ...
  const flush=()=>{ if(!busy || outstanding || !pending)return; const progress=pending;pending=null;outstanding=++sequence;scope.postMessage({version:5,id:currentId,type:'progress',sequence,progress}); };
  scope.onmessage=event=>{
    const v=event.data;
    if(shape(v,['version','id','type','sequence']) && v.version===5 && v.type==='ack' && v.id===currentId && v.sequence===outstanding){outstanding=0;flush();return;}
    if(!shape(v,['version','id','action']) || v.version!==5 || !int(v.id,1) || v.id<=lastId || busy)return;
      ...
      try{
        ...
        if(validWorldAction(action)){
          ...
          try{
            ...
            if(result){scope.postMessage({version:5,id,type:'result',result});return;}
          }catch(error){
            ...
            scope.postMessage({version:5,id,type:'result',result:{type:'world-rejection',modelHash:world.model.sha256,revision:world.revision,code}});return;
          }
        }
        if(action.type==='pick'){
          if(!frame || action.frameId!==frameId || action.x>=frame.viewport.width || action.y>=frame.viewport.height)throw new Error('stale-frame');
          scope.postMessage({version:5,id,type:'result',result:{type:'pick',frameId,selection:frame.pick(action.x,action.y)}});
        }else{
          ...
          scope.postMessage({version:5,id,type:'result',result:{type:'frame',controlPoints,world:snapshot,frameId,camera:{cameraX,cameraY,zoom,width,height},summary,allocations:{...frame.allocations,voxel:frame.allocations.voxel??null},rgba}},[rgba]);
        }
      }catch(error){
        ...
        scope.postMessage({version:5,id,type:'error',code});
