...
export interface ViewportScene { render(viewport:TerrainViewport):ViewportFrame }
export type SceneLoader = (files: File[], profile: TerrainProfile, progress: (p: TerrainProgress)=>void) => Promise<{ scene: ViewportScene; summary: SceneSummary }>;
...
export function attachTerrainWorker(scope: TerrainScope, load: SceneLoader): void {
  ...
  const flush=()=>{ if(!busy || outstanding || !pending)return; const progress=pending;pending=null;outstanding=++sequence;scope.postMessage({version:2,id:currentId,type:'progress',sequence,progress}); };
  scope.onmessage=event=>{
    const v=event.data;
    if(shape(v,['version','id','type','sequence']) && v.version===2 && v.type==='ack' && v.id===currentId && v.sequence===outstanding){outstanding=0;flush();return;}
    if(!shape(v,['version','id','action']) || v.version!==2 || !int(v.id,1) || v.id<=lastId || busy)return;
    ...
    void(async()=>{
      try{
        ...
        if(action.type==='pick'){
          ...
          scope.postMessage({version:2,id,type:'result',result:{type:'pick',frameId,selection:frame.pick(action.x,action.y)}});
        }else{
          ...
          scope.postMessage({version:2,id,type:'result',result:{type:'frame',frameId,camera,summary,allocations:{...frame.allocations},rgba}},[rgba]);
        }
      }catch(error){
        ...
        scope.postMessage({version:2,id,type:'error',code});
