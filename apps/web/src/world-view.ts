...
import { worldText } from './world-i18n.ts';
...
export function mountWorld(root: HTMLElement, controller: TerrainController): () => void {
  ...
  const unsubscribe = controller.subscribe(state => {
    ...
    get('events').textContent = world.events.map(e => `${e.tick} · #${e.entityId} · ${e.phase}/${e.kind}${e.cell === null ? '' : ` · ${e.cell % 512}, ${Math.floor(e.cell / 512)}`}`).join('\n') + (world.omittedEvents ? `\n${t('omitted')}: ${world.omittedEvents}` : '');
