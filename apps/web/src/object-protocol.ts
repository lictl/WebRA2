...
import { validVoxelSummary, validVoxelPickSource, type VoxelArtworkSummary, type VoxelPickSource } from './voxel-protocol.ts';
...
export type ArtworkType = { id: string; name: string; family: ObjectFamily; format: 'shp' | 'voxel'; status: ArtworkStatus; placements: number; rendered: number; reasons: string[]; omittedReasons: number; sourcePath: string | null; sourceHash: string | null; palettePath: string | null; paletteHash: string | null };
export type ArtworkSummary = { policy: 'webra2-object-still-2'; presentation: typeof PLACED_STILL_POLICY; voxel: VoxelArtworkSummary | null; types: number; rendered: number; unavailable: number; assets: number; palettes: number; sourceBytes: number; decodedBytes: number; indexedFrames: number; rows: ArtworkType[]; omittedTypes: number; omittedPlacements: number; omittedRendered: number; truncatedFields: number; unplaced: number };
export type ObjectInfo = { id: string; typeId: string; name: string; family: ObjectFamily; owner: string | null; format: 'shp' | 'voxel'; voxel: VoxelPickSource | null; x: number; y: number; frame: 0; sourcePath: string; sourceHash: string; palettePath: string; paletteHash: string };
...
export function validArtworkSummary(v: unknown, placements: number): v is ArtworkSummary {
  if (!record(v,['policy','presentation','voxel','types','rendered','unavailable','assets','palettes','sourceBytes','decodedBytes','indexedFrames','rows','omittedTypes','omittedPlacements','omittedRendered','truncatedFields','unplaced']) || v.policy !== 'webra2-object-still-2' || v.presentation !== PLACED_STILL_POLICY || !validVoxelSummary(v.voxel)) return false;
  ...
  for (const row of v.rows) {
    if (!record(row,['id','name','family','format','status','placements','rendered','reasons','omittedReasons','sourcePath','sourceHash','palettePath','paletteHash']) || !id(row.id) || ids.has(row.id) || !text(row.name,ART_REPORT_LIMIT.label) || !familyValues.some(f=>row.family===f) || !['shp','voxel'].some(f=>row.format===f) || !statusValues.some(s=>row.status===s) || !nat(row.placements) || !nat(row.rendered) || row.rendered > row.placements || (row.status !== 'ready' && row.rendered !== 0) || !dense(row.reasons,ART_REPORT_LIMIT.reasons) || !row.reasons.every(r=>text(r,128)) || !nat(row.omittedReasons) || !sourcePair(row.sourcePath,row.sourceHash) || !sourcePair(row.palettePath,row.paletteHash)) return false;
  ...
  if (v.voxel && (v.voxel.rendered > v.rendered || v.voxel.placements > placements)) return false;
...
export function validObjectPick(v: unknown): v is ObjectPick {
  ...
  const o=v.object;
  if (!record(o,['id','typeId','name','family','owner','format','voxel','x','y','frame','sourcePath','sourceHash','palettePath','paletteHash']) || !id(o.id) || !id(o.typeId) || !text(o.name,ART_REPORT_LIMIT.label) || !familyValues.some(f=>o.family===f) || !(o.owner===null || text(o.owner,ART_REPORT_LIMIT.label)) || !nat(o.x,511) || o.x===0 || !nat(o.y,511) || o.y===0 || o.frame !== 0 || !text(o.sourcePath,ART_REPORT_LIMIT.path) || !digest(o.sourceHash) || !text(o.palettePath,ART_REPORT_LIMIT.path) || !digest(o.paletteHash)) return false;
  if (o.format === 'shp' ? o.voxel !== null : o.format !== 'voxel' || !validVoxelPickSource(o.voxel)) return false;
  return nat(v.canvasX,2047) && nat(v.canvasY,2047) && [v.worldX,v.worldY].every(n=>typeof n==='number' && Number.isSafeInteger(n) && Math.abs(n)<=2097152) && typeof v.depth === 'number' && Number.isFinite(v.depth) && Math.abs(v.depth)<=2097152 && (o.format==='voxel' || Number.isSafeInteger(v.depth));
}
