...
import { saveText } from './practice-protocol.ts';
export type SaveSlot = 1 | 2 | 3;
export interface PracticeStorage { read(slot: SaveSlot, signal: AbortSignal): Promise<string | null>; write(slot: SaveSlot, text: string, signal: AbortSignal): Promise<void>; remove(slot: SaveSlot, signal: AbortSignal): Promise<void> }
  ...
  #transaction(slot: SaveSlot, operation: 'get' | 'put' | 'delete', text: string | undefined, signal: AbortSignal): Promise<string | null> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const open = this.factory.open('webra2-practice-v1', 1);
