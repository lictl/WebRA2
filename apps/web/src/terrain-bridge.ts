  ...
  request(action:TerrainAction,signal:AbortSignal,progress?:(p:TerrainProgress)=>void):Promise<TerrainResult>{
    ...
    return new Promise((resolve,reject)=>{
      ...
      const message=(event:Event)=>{
        ...
        if(shape(v,['version','id','type','sequence','progress']) && v.version===3 && v.type==='progress' && action.type==='load' && int(v.sequence,1) && v.sequence>lastProgress && validProgress(v.progress)){
          lastProgress=v.sequence;try{progress?.(v.progress);if(!settled)this.worker.postMessage({version:3,id,type:'ack',sequence:v.sequence});}catch{failed();}return;
        }
        if(shape(v,['version','id','type','code']) && v.version===3 && v.type==='error' && code(v.code)){finish(new Error(v.code));return;}
        if(!shape(v,['version','id','type','result']) || v.version!==3 || v.type!=='result' || !validResult(v.result)){finish(new Error('invalid'));return;}
        ...
        if(action.type==='pick'){
          ...
          const identity=result.summary.profile+':'+result.summary.mapHash+':'+result.summary.contentHash+':'+result.summary.artwork.presentation+':'+(result.summary.world?.modelHash??'none');
      ...
      try{this.worker.postMessage({version:3,id,action});}catch{failed();}
    });
  }
