...
export type TerrainProgress = { phase: 'scan' | 'verify' | 'definitions' | 'mission' | 'theater' | 'tiles' | 'compose'; completed: number; total: number; bytes: number };
export type SceneSummary = { profile: TerrainProfile; mission: string; mapHash: string; paletteHash: string; cells: number; objects: number; assets: number; verifiedBytes: number; sourceBytes: number; decodedBytes: number; decodedSlots: number; bounds: { x: number; y: number; width: number; height: number }; diagnostics: { code: string; count: number }[] };
export type FrameSummary = { rgbaBytes: number; depthBytes: number; ownerBytes: number; totalPixelBytes: number; samples: number };
...
export type PickResult = { type: 'pick'; frameId: number; cell: CellPick | null };
export type TerrainResult = FrameResult | PickResult;
export type TerrainReply = { version: 1; id: number; type: 'progress'; sequence: number; progress: TerrainProgress } | { version: 1; id: number; type: 'result'; result: TerrainResult } | { version: 1; id: number; type: 'error'; code: string };
...
export function validProgress(v: unknown): v is TerrainProgress { return shape(v,['phase','completed','total','bytes']) && ['scan','verify','definitions','mission','theater','tiles','compose'].some(p=>v.phase===p) && int(v.completed,0,2**34) && int(v.total,0,2**34) && v.completed <= v.total && int(v.bytes,0,2**34); }
export function validScene(v: unknown): v is SceneSummary {
  if (!shape(v,['profile','mission','mapHash','paletteHash','cells','objects','assets','verifiedBytes','sourceBytes','decodedBytes','decodedSlots','bounds','diagnostics']) || !profileId(v.profile) || v.mission !== (v.profile==='ra2'?'all01t.map':'all01umd.map') || !hash(v.mapHash) || !hash(v.paletteHash)) return false;
  if (!int(v.cells,1,130816) || !int(v.objects,0,262144) || !int(v.assets,1,1024) || !int(v.verifiedBytes,0,2**34) || !int(v.sourceBytes,0,128*1024**2) || !int(v.decodedBytes,0,64*1024**2) || !int(v.decodedSlots,0,65536)) return false;
  return shape(v.bounds,['x','y','width','height']) && number(v.bounds.x) && number(v.bounds.y) && int(v.bounds.width,1,2097152) && int(v.bounds.height,1,2097152) && rows(v.diagnostics,32) && v.diagnostics.every(d=>shape(d,['code','count']) && code(d.code) && int(d.count,0,2**34));
}
export function validPick(v: unknown): v is CellPick | null { return v === null || (shape(v,['sourceRecord','x','y','assetId','subtile','worldX','worldY','depth']) && int(v.sourceRecord,0,130815) && int(v.x,1,511) && int(v.y,1,511) && typeof v.assetId==='string' && /^[A-Za-z0-9][A-Za-z0-9._:/#-]{0,255}$/.test(v.assetId) && int(v.subtile,0,255) && number(v.worldX) && number(v.worldY) && number(v.depth,2097152)); }
export function validResult(v: unknown): v is TerrainResult {
  if (shape(v,['type','frameId','cell']) && v.type==='pick') return int(v.frameId,1) && validPick(v.cell);
  ...
  return shape(a,['rgbaBytes','depthBytes','ownerBytes','totalPixelBytes','samples']) && a.rgbaBytes===bytes && a.depthBytes===bytes && a.ownerBytes===bytes && a.totalPixelBytes===bytes*3 && int(a.samples,0,64*1024**2) && v.rgba instanceof ArrayBuffer && Object.getPrototypeOf(v.rgba)===ArrayBuffer.prototype && !Object.getOwnPropertyDescriptor(ArrayBuffer.prototype,'resizable')?.get?.call(v.rgba) && Reflect.ownKeys(v.rgba).length===0 && v.rgba.byteLength===bytes;
}
