...
export function validateGraphSource(s: GraphSource): void {
  if (!s.rootFile || /[/\\\0]/.test(s.rootFile) || s.rootFile === '.' || s.rootFile === '..' || !/^[a-f0-9]{64}$/.test(s.rootSha256) || !/^[a-f0-9]{64}$/.test(s.sha256)) fail('graph-source-identity');
...
export function compileCampaignGraph(profile: 'ra2' | 'yr', input: GraphDocument[], limits: GraphLimits = GRAPH_LIMITS): CampaignGraph {
  ...
  for (const doc of docs) for (const row of doc.document.entries) {
    const kind = lists[symbol(row.section)];
  ...
  for (let d = 0; d < docs.length; d++) {
    ...
    for (const section of doc.document.sections) {
      ...
      if (kinds && !lists[symbol(section.name)]) for (const kind of kinds) {
    ...
    for (const row of doc.document.entries) {
      const section = symbol(row.section), kind = rowKinds[section];
  ...
  for (const row of docs[missionIndex]!.document.entries) {
    const kind = lists[symbol(row.section)], at = location(missionIndex, row.line);
  ...
  for (const [id, rows] of nodeRows) {
    ...
    for (const row of rows) {
      ...
      const targetKind = fields[n.kind]?.[key];
      if (targetKind) {
        const values = key === 'allies' ? tokens(row, source, 1) : [censusValue(row.value)];
        for (const value of values ?? []) reference(id, targetKind, value, `${n.kind}-${key}`, at, key === 'tag');
      }
      ...
      if (n.kind === 'trigger') {
        ...
        if (parts) { reference(id, 'house', parts[0]!, 'trigger-house', at, true); reference(id, 'trigger', parts[1]!, 'attached-trigger', at, true); }
      ...
      if (n.kind === 'ai-trigger') {
        ...
        if (parts) {
          ...
          if (parts[2]!.toLowerCase() !== '<all>') reference(id, 'house', parts[2]!, 'ai-owner', at, true);
  ...
  return { profile, sources: docs.map(d => ({ role: d.role, identity: { ...d.source } })), nodes, edges, diagnostics, reachable: [...reachable].sort((a, b) => a - b), cycles,
