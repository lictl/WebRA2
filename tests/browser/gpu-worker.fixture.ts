...
export class GpuOriginalWorker extends EventTarget {
  ...
  constructor(refusal?:'voxel-layer'|'scene-unavailable'|'scene-budget',withSprite=false,withVoxel=false){
    super();
    ...
    if(withVoxel){summary.artwork.types++;summary.artwork.omittedTypes++;summary.artwork.omittedRendered++;summary.artwork.rendered++;summary.artwork.unavailable--;
      summary.artwork.voxel={policy:'webra2-voxel-resources-1',presentation:'webra2-world-voxel-still-1',types:1,readyTypes:1,placements:1,rendered:1,unavailable:0,assets:1,parts:3,sourceBytes:100,geometryBytes:15,selectedVoxels:3,matrices:3};}
    const project=(snapshot?:WorldSnapshot)=>{const a=snapshot?.actors.find(a=>a.id===2);return {objects:batch.objects.map(o=>{const a=snapshot?.actors[0];return a?{...o,x:a.x*20,y:a.y*10}:o;}),retiredObjectIds:[],
      ...(withVoxel?{voxelPlacements:[{objectId:'object-1',x:a?.x??7,y:a?.y??5,column:(a?.x??7)-1,row:(a?.y??5)-1,elevation:0}]}:{})};};
    const loader:SceneLoader=async()=>({world,summary,scene:{
      locate:(x,y)=>({x:x*20,y:y*10}),
      ...(refusal&&refusal!=='scene-budget'?{gpuRefusal:refusal}:{gpu:()=>{this.gpuPreparations++;if(refusal==='scene-budget')throw new Error('gpu-raster-budget');return {scene:compileGpuScene(terrain,batch),objectInfo:withSprite?[gpuOriginalInfo]:[],project,...(withVoxel?{voxel:originalVoxelResources(world.model.sha256)}:{})};}}),
      render:(viewport,snapshot)=>{this.cpuRenders++;const frame=terrain.renderSprites(viewport,{...batch,objects:project(snapshot).objects});return {...frame,allocations:{...frame.allocations,objects:frame.allocations.objects+(withVoxel?1:0)},pick:()=>null};}
    }});
