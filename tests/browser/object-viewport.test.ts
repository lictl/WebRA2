...
test('object replies require bounded exact records, joined totals, fixed buffers and current v4 envelopes',async()=>{
  ...
  worker.emit({version:3,id:1,type:'result',result:frame});await assert.rejects(pending,/invalid/);assert.equal(worker.terminated,1);
});
