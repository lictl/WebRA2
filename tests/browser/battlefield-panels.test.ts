...
class Node extends EventTarget {
  hidden=false; attributes=new Map<string,string>(); focused=0; value=''; scrollTop=0;
...
test('optional panels keep form values, pause explicitly and return keyboard focus to the map',()=>{
 ...
 const body=f.nodes.get('.world-panel-body')!;
 for(let i=0;i<20;i++){body.scrollTop=200;f.click('open-saves');assert.equal(body.scrollTop,0);assert.equal(saves.hidden,false);assert.equal(orders.hidden,true);f.click('open-diagnostics');assert.equal(saves.hidden,true);assert.equal(diagnostics.hidden,false);body.scrollTop=700;f.click('return-diagnostics');assert.equal(body.scrollTop,0);assert.equal(orders.hidden,false);assert.equal(diagnostics.hidden,true);assert.equal(saves.value,'saved local slot choice');}
