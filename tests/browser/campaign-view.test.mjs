...
test('mounted chooser wires local selection, profiles, translated launches, back, retry and unavailable entries',async()=>{
 ...
 try{
  ...
  const launch=get('entries').querySelectorAll('button[data-launch]')[1];assert.match(launch.attributes['aria-label'],/Soviet/);launch.click();await settle();assert.equal(controller.state.phase,'ready');assert.equal(get('chooser').hidden,true);assert.equal(get('viewport').textContent,'Original viewport fixture');
  await controller.backCampaigns();await settle();assert.equal(get('chooser').hidden,false);assert.equal(controller.state.files,1);assert.equal(calls.filter(a=>a.type==='campaign-scan').length,1);
