...
const summary:SceneSummary={profile:'ra2',mission:'all01t.map',mapHash:'a'.repeat(64),paletteHash:'b'.repeat(64),cells:3,objects:0,assets:1,verifiedBytes:100,sourceBytes:100,decodedBytes:50,decodedSlots:1,bounds:{x:0,y:0,width:120,height:45},diagnostics:[{code:'native-composition-unverified',count:1}]};
const view:Camera={cameraX:0,cameraY:0,zoom:1,width:120,height:45};
function frame(frameId=1,camera=view):FrameResult{const bytes=camera.width*camera.height*4;return {type:'frame',frameId,camera,summary:structuredClone(summary),allocations:{rgbaBytes:bytes,depthBytes:bytes,ownerBytes:bytes,totalPixelBytes:bytes*3,samples:1},rgba:new ArrayBuffer(bytes)};}
...
test('bridge admits one request, validates progress and binds result/pick to exact request and displayed frame',async()=>{
  ...
  worker.emit({version:1,id:99,type:'result',result:frame(99)});worker.emit({version:1,id:1,type:'progress',sequence:1,progress:{phase:'scan',completed:1,total:2,bytes:10}});
  assert.equal(worker.sent.length,2);assert.deepEqual(progress,[1]);worker.emit({version:1,id:1,type:'result',result:frame(1)});assert.equal((await first).type,'frame');
  const picking=bridge.request({type:'pick',frameId:1,x:2,y:2},signal);worker.emit({version:1,id:2,type:'result',result:{type:'pick',frameId:1,cell:null}});assert.deepEqual(await picking,{type:'pick',frameId:1,cell:null});
  const rendering=bridge.request({type:'render',camera:{...view,cameraX:32}},signal);worker.emit({version:1,id:3,type:'result',result:frame(3)});await assert.rejects(rendering,/invalid/);assert.equal(worker.terminated,1);
});
test('bridge rejects cross-profile metadata, stale pick identity and malformed terminal shape; abort terminates promptly',async()=>{
  for(const invalid of [{...frame(),frameId:2},{...frame(),summary:{...summary,profile:'yr',mission:'all01umd.map'}},{...frame(),summary:{...summary,diagnostics:[null]}}]){const worker=new FakeWorker(),bridge=new TerrainBridge(worker),promise=bridge.request(load,new AbortController().signal);worker.emit({version:1,id:1,type:'result',result:invalid});await assert.rejects(promise,/invalid/);assert.equal(worker.terminated,1);}
  const worker=new FakeWorker(),bridge=new TerrainBridge(worker),abort=new AbortController();const promise=bridge.request(load,abort.signal);abort.abort();await assert.rejects(promise,{name:'AbortError'});worker.emit({version:1,id:1,type:'result',result:frame()});assert.equal(worker.terminated,1);
});
...
test('camera requests coalesce; outdated pixels and picks are never published for the displayed frame',async()=>{
  ...
  const picking=controller.pick(1,1);controller.pan(4,0);p.calls[3]!.response.resolve({type:'pick',frameId:3,cell:{sourceRecord:0,x:1,y:2,assetId:'tile',subtile:0,worldX:61,worldY:1,depth:0}});await picking;
  assert.equal(controller.state.cell,null);assert.equal(p.calls.length,5);assert.equal(p.calls[4]!.action.type,'render');controller.cancel();p.calls[4]!.response.resolve(frame(5));await tick();assert.equal(controller.state.frame,null);
});
...
function fakeScene():TerrainScene{return {render(viewport){const bytes=viewport.width*viewport.height*4,rgba=new Uint8Array(bytes).fill(17);return {rgba,viewport,allocations:{rgbaBytes:bytes,depthBytes:bytes,ownerBytes:bytes,totalPixelBytes:bytes*3,samples:1},pick(x,y){return {sourceRecord:0,x:1,y:2,assetId:'tile',subtile:0,worldX:viewport.cameraX+x/viewport.zoom,worldY:viewport.cameraY+y/viewport.zoom,depth:0};}};}} as TerrainScene;}
test('worker retains scene/picking, transfers only bounded RGBA, preserves explicit folder paths and progress backpressure',async()=>{
  const messages:TerrainReply[]=[],ready=deferred<{scene:TerrainScene;summary:SceneSummary}>();let inheritedPath='';
  ...
  scope.onmessage!({data:structuredClone({version:1,id:1,action:load})});assert.equal(inheritedPath,'game/sample.mix');assert.equal(messages.length,1);
  scope.onmessage!({data:{version:1,id:1,type:'ack',sequence:1}});assert.equal(messages.length,2);assert.equal((messages[1] as {progress:{completed:number}}).progress.completed,3);
  ...
  scope.onmessage!({data:{version:1,id:2,action:{type:'pick',frameId:1,x:30,y:10}}});await tick();assert.equal(messages.at(-1)!.type,'result');
  scope.onmessage!({data:{version:1,id:3,action:{type:'render',camera:view}}});await tick();scope.onmessage!({data:{version:1,id:4,action:{type:'pick',frameId:1,x:30,y:10}}});await tick();assert.deepEqual(messages.at(-1),{version:1,id:4,type:'error',code:'stale-frame'});
});
test('worker rejects malformed actions and failed preparation without exposing exception text or partial scene',async()=>{
  ...
  scope.onmessage!({data:{version:1,id:1,action:{...load,width:961}}});await tick();assert.equal(attempts,0);assert.equal(messages.at(-1)!.type,'error');
  scope.onmessage!({data:{version:1,id:2,action:load}});await tick();assert.deepEqual(messages.at(-1),{version:1,id:2,type:'error',code:'unavailable'});
  scope.onmessage!({data:{version:1,id:3,action:{type:'render',camera:view}}});await tick();assert.equal(messages.at(-1)!.type,'error');
});
