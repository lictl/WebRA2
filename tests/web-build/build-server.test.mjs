...
async function fixture() {
  ...
  for (const name of ['LICENSE', 'apps/web/PROVENANCE.md', 'LICENSES/GPL-3.0-or-later.txt', 'docs/licensing.md',
    'packages/render/GPU_PROVENANCE.md', 'packages/render/GPU_VOXEL_PROVENANCE.md',
...
test('build emits portable code and notices with a hash manifest, without retail inputs', async () => {
  ...
  try {
    ...
    for(const name of ['GPU_PROVENANCE.md','GPU_VOXEL_PROVENANCE.md'])assert.equal(await readFile(join(root,'dist/licenses/packages-render-'+name+'.txt'),'utf8'),await readFile(join(repositoryRoot,'packages/render',name),'utf8'));
