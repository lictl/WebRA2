...
test('build emits portable code and notices with a hash manifest, without retail inputs', async () => {
  ...
  try {
    ...
    assert.equal(await readFile(join(root, 'dist/licenses/packages-sim-MISSION_TEAM_CONSTRUCTOR_PROVENANCE.md.txt'), 'utf8'), await readFile(join(repositoryRoot, 'packages/sim/MISSION_TEAM_CONSTRUCTOR_PROVENANCE.md'), 'utf8'));
