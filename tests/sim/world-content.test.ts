...
function fixture({profile='ra2' as 'ra2'|'yr',extraRules='',player='Commander'}={}) {
  ...
  return {mapBytes,rules,definitions,traversal};
}
test('both profiles account for every placement with stable rows, typed health, owners and explicit movement coverage',()=>{
 for(const profile of ['ra2','yr'] as const){const r=compileWorldContent(fixture({profile}));
  ...
  assert.deepEqual(r.coverage,{placements:6,mobile:2,stationary:1,passive:2,unavailable:1,sharedAnchors:0});
...
test('lower bounds reject before publishing a partial world and cannot be raised',()=>{
 const f=fixture();for(const key of ['mapBytes','entities','players','grids'] as const){assert.throws(()=>compileWorldContent(f,{[key]:0}));assert.throws(()=>compileWorldContent(f,{[key]:WORLD_CONTENT_LIMITS[key]+1}));}
