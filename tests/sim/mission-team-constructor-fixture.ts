...
export function constructorFixture(profile: 'ra2' | 'yr', extra = '', script = '0=3,1\n1=50,5', infantry = false, options: { transfers?: boolean; count?: number } = {}) {
  ...
  const f = missionTeamFixture({ profile, ...(options.transfers ? { extraMap } : {}), ...(options.count ? { count: options.count } : {}), forceType: infantry ? 'Walker' : 'Carrier', script, waypoint: '0=3003\n1=3004', extraRules: rules });
