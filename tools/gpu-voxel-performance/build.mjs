...
import { noticeSources } from '../performance/notices.mjs';
...
export async function buildGpuVoxelPerformance(destination, mode = 'voxel', root = repositoryRoot) {
  if (mode !== 'voxel') throw Error('Invalid mode');
  ...
  const result = await build({ absWorkingDir: root, entryPoints: {main:'tools/gpu-voxel-performance/main.mjs'},
    outdir, bundle: true, format: 'esm', platform: 'browser', target: ['es2022'], minify: true,
    ...
    plugins: [{ name: 'performance-code-only', setup(builder) {
      builder.onLoad({ filter: /.*/ }, async args => {
        ...
        if (!/^(?:tools\/(?:gpu-voxel-performance|gpu-performance|performance)\/|tests\/render\/gpu-voxel-fixtures\.ts$|apps\/web\/src\/|packages\/|node_modules\/(?:@noble\/hashes|egoroof-blowfish)\/)/.test(name)
  ...
  files.set('NOTICES.txt', Buffer.from('Original WebRA2 GPU voxel performance harness: GPL-3.0-or-later. Existing GPL/MIT engine components and locked dependency notices are included.\nThe broader app notice set is retained intentionally; inclusion does not mean every component executes in this workload.\nNo retail assets are included. This is a local diagnostic, not a finalized distribution.\nCorresponding source, original fixtures and build instructions: https://github.com/lictl/WebRA2 (tools/gpu-voxel-performance; docs/gpu-voxel-browser.md).\n'));
  ...
  const manifest = { schema: 1, mode, minified: true, target: 'es2022', gitHead, engineDiff: engineDiff ? engineDiff.split('\n') : [],
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (process.argv.length < 3 || process.argv.length > 4) throw Error('Usage: node tools/gpu-voxel-performance/build.mjs local/<fresh-output-directory> [voxel]');
  const manifest = await buildGpuVoxelPerformance(process.argv[2],process.argv[3]??'voxel');
