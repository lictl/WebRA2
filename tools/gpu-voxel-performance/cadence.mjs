...
// Only the bounded original fixture calls this helper. The source factory owns a fresh plane copy.
export function writeVoxelMatrixPlane(instances,phase,plane){const dx=Math.sin(phase*.7)*5,dy=Math.cos(phase*.9)*3;for(let i=0;i<instances.length;i++){const m=instances[i].modelToView,at=i*12;for(let n=0;n<12;n++)plane[at+n]=m[n];plane[at+3]+=dx+(i%3-1)*Math.sin(phase)*.25;plane[at+7]+=dy;}}
...
export async function runVoxelCadence({canvas,gl,token,report,check,status,options}){
 const {groups,width,height,warmupMs,measureMs}=options;Object.assign(report,options,{rendererOnly:true,worldSimulation:false,preparation:'retained-layout-owned-matrix-plane'});
 ...
 try{
  const layoutStart=performance.now(),layout=createGpuVoxelInstanceLayout(workload.scene,workload.reference.instances),matrices=new Float64Array(workload.reference.instances.length*12),layoutEnd=performance.now();report.layoutAllocations=layout.allocations;report.callerMatrixBytes=matrices.byteLength;
  ...
  report.cold={fixtureMs:readyFixture-startFixture,layoutAndCallerPlaneMs:layoutEnd-layoutStart,loadSubmitMs:afterLoad-beforeLoad,firstDrawSubmitMs:afterFirst-beforeFirst,firstObservedElapsedMs:cold.observedAt-beforeFirst,gpuMs:cold.gpuMs};
  ...
  await loop((raf,now)=>{if(now>=measuredEnd)return true;opportunities.push(raf);if(timing.pending.length>=8){timing.skipped++;return false;}
   const id=++next,begin=performance.now(),phase=(begin-warmupStart)/1000;
   writeVoxelMatrixPlane(workload.reference.instances,phase,matrices);
   lastFrame=prepareGpuVoxelLayoutFrame(layout,{matrices,width,height});const preparedAt=performance.now(),submittedAt=performance.now(),measured=submittedAt>=measuredStart&&submittedAt<measuredEnd;
