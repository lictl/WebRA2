...
export async function serveGpuVoxelPerformance(directory, port = 4223, isolated = false, root = repositoryRoot) {
  ...
  if(manifest.mode !== 'voxel')throw Error('Invalid mode');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (process.argv.length < 3 || process.argv.length > 5) throw Error('Usage: node tools/gpu-voxel-performance/serve.mjs local/<build> [port] [isolated]');
  const port = process.argv[3] === undefined ? 4223 : Number(process.argv[3]);
  if(process.argv[4]!==undefined&&process.argv[4]!=='isolated')throw Error('Invalid header mode');
  await serveGpuVoxelPerformance(process.argv[2], port, process.argv[4]==='isolated'); console.log(`Immutable GPU diagnostic: http://127.0.0.1:${port}/`);
}
