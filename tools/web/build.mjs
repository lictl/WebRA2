...
const allowedOutput = /^(?:app\.(?:js|css)|workers\/(?:import|simulation)\.js|chunks\/[a-zA-Z0-9_-]+\.js)$/;
...
export async function buildWeb(root = repositoryRoot) {
  ...
  const result = await build({
    absWorkingDir: root, entryPoints: { app: 'apps/web/src/main.ts', 'workers/import': 'apps/web/src/import-worker.ts', 'workers/simulation': 'apps/web/src/simulation-worker.ts' }, outdir: outputDirectory,
