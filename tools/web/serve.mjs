...
const approved = /^(?:index\.html|app\.(?:js|css)|workers\/(?:import|simulation)\.js|chunks\/[a-zA-Z0-9_-]+\.js|LICENSE\.txt|NOTICES\.txt|LICENSES\/GPL-3\.0-or-later\.txt|licenses\/[a-zA-Z0-9_.-]+\.txt)$/;
...
export async function startWebServer({ root = repositoryRoot, port = 4173 } = {}) {
  ...
  if (!files.has('/index.html') || !files.has('/app.js') || !files.has('/workers/import.js') || !files.has('/workers/simulation.js')) throw new Error('Missing app entrypoints');
  let authority;
