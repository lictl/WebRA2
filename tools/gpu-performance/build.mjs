...
export async function buildGpuPerformance(destination, mode = 'full', root = repositoryRoot) {
  if (!['refresh','full'].includes(mode)) throw Error('Invalid mode');
  ...
  const result = await build({ absWorkingDir: root, entryPoints: mode === 'refresh' ? {main:'tools/gpu-performance/refresh.mjs'} : {main:'tools/gpu-performance/main.mjs',worker:'tools/gpu-performance/simulation-worker.mjs'},
  ...
  files.set('NOTICES.txt', Buffer.from('Original WebRA2 GPU performance harness: GPL-3.0-or-later. Existing GPL/MIT engine components and locked dependency notices are included.\nThe broader app notice set is retained intentionally; inclusion does not mean every component executes in this workload.\nNo retail assets are included. This is a local diagnostic, not a finalized distribution.\nCorresponding source, original fixtures and build instructions: https://github.com/lictl/WebRA2 (tools/gpu-performance; docs/gpu-renderer-experiment.md).\n'));
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (process.argv.length < 3 || process.argv.length > 4) throw Error('Usage: node tools/gpu-performance/build.mjs local/<fresh-output-directory> [refresh|full]');
  const manifest = await buildGpuPerformance(process.argv[2],process.argv[3]??'full');
