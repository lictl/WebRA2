...
function dense(value,max){return Array.isArray(value)&&value.length<=max&&Reflect.ownKeys(value).length===value.length+1&&Array.from({length:value.length},(_,i)=>Object.getOwnPropertyDescriptor(value,String(i))).every(d=>d&&'value'in d);}
export function validSnapshot(v){return keys(v,['nextTick','modelHash','stateHash','queued','actors'])&&Number.isInteger(v.nextTick)&&number(v.nextTick,1800)&&sha(v.modelHash)&&sha(v.stateHash)&&Number.isInteger(v.queued)&&number(v.queued,256)&&dense(v.actors,1024)&&v.actors.length<=1024&&v.actors.length>=64&&Reflect.ownKeys(v.actors).length===v.actors.length+1&&v.actors.every((a,i)=>keys(a,['id','x','y','progress','moving'])&&a.id===i+1&&Number.isInteger(a.x)&&number(a.x,511)&&Number.isInteger(a.y)&&number(a.y,511)&&Number.isInteger(a.progress)&&number(a.progress)&&typeof a.moving==='boolean');}
